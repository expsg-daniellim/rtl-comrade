"""Unit tests for loader_plugin.py — plugin discovery and loading."""
...
from rtl_comrade.loader_plugin import (
	PluginModuleConfig,
	load_plugin_config,
	load_plugin_configs,
	load_plugins,
...
def test_plugin_file_config_load_spec_none(logging_handler, tmp_path):
	...
	plugin_file.write_text(_SIMPLE_PLUGIN)
	config = PluginFileConfig(name=None, file=plugin_file, type_=None, plugins=None)
	with patch("rtl_comrade.loader_utils.importlib.util.spec_from_file_location", return_value=None):
		with pytest.raises(typer.Exit):
			config.load()
...
def test_plugin_file_config_load_spec_loader_none(logging_handler, tmp_path):
	...
	with patch(
		"rtl_comrade.loader_utils.importlib.util.spec_from_file_location",
...
def _exec_raising_patches(exc):
	...
	return (
		...
		patch(
			"rtl_comrade.loader_utils.importlib.util.module_from_spec",
...
def test_load_plugin_config_does_not_load(logging_handler, tmp_path):
	plugin_file = tmp_path / "mods.py"
	plugin_file.write_text(_SIMPLE_PLUGIN)
	with patch("rtl_comrade.loader_utils.importlib.util.spec_from_file_location") as mock_spec:
