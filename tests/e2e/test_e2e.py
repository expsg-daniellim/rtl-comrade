...
def _import_parse_log_mod():
    """Import ParseLogMod directly from the module file."""
    spec = importlib.util.spec_from_file_location(
        "modules_rtl_buddy_sim",
        _PROJECT_ROOT / "modules" / "rtl_buddy" / "sim.py",
    )
    assert spec is not None and spec.loader is not None
    mod = importlib.util.module_from_spec(spec)
    sys.path.insert(0, str(_PROJECT_ROOT))
    spec.loader.exec_module(mod)
    return mod.ParseLogMod
...
def test_parselog_fail_wins_over_pass(tmp_path):
    ...
    """
    sys.path.insert(0, str(_PROJECT_ROOT))
    ParseLogMod = _import_parse_log_mod()
