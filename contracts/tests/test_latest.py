...
async def test_empty_state_queue_during_drain():
    ...
    await run_contract_scenario(
        LatestContract,
        port_inputs={
            "state": [PortTestInput(10, delay=1), PortTestInput(EndSentinel("src"), delay=3)],
            "trigger": [PortTestInput(1, delay=2), PortTestInput(EndSentinel("src"), delay=4)],
        },
        expected_outputs=[{"state": 10, "trigger": 1}, EndSentinel],
        config=_CFG,
    )
...
async def test_all_trigger_ports_already_ended():
    ...
    await run_contract_scenario(
        ...
        expected_outputs=[EndSentinel, EndSentinel],
        config=_CFG,
    )
