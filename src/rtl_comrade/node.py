...
class Node:
	...
	def __init__(
		...
		if "config" in module_init_sig.parameters:
			if hasattr(Module, "Config"):
				try:
					...
					log.fatal(
						"config.deserialise.serde_error",
						context="harness.node.module",
						node=self.id,
						module=Module.__name__,
						exc_info=e,
		...
		try:
			...
			log.fatal(
				"invalid_tuple",
				context="harness.node.module.emits",
				lineno=e.lineno,
				tuple_=e.tuple_,
			)
	...
	async def process_result(self, res: tuple[str, Any] | Any):
		...
		if isinstance(res, tuple):
			if len(res) != 2:
				log.error(
					"malformed_output",
					context="harness.module.res",
					port=str(res[0]) if len(res) > 0 else None,
					data_type=type(res).__name__,
					data_repr=repr(res),
				)
				return
