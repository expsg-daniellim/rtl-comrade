# module refers to a Python module
...
def load_module(config:ModuleFileConfig):
	...
	if not config.file.is_file():
		raise LoadException(module_name, f"{config.file} not found")

	spec = importlib.util.spec_from_file_location(module_name, config.file)
	if spec is None:
		raise LoadException(module_name, "spec could not be created")

	module = importlib.util.module_from_spec(spec)
	if spec.loader is None:
		raise LoadException(module_name, "spec has no loader")
	...
	for mod in to_get:
		if mod.class_name in available_mods:
			if mod.name in res:
				raise LoadException(module_name, f"duplicate module definition {mod.name}")
			res[mod.name] = available_mods[mod.class_name]
		else:
			raise LoadException(module_name, f"module {mod.name} class {mod.class_name} not found in {config.file}")
