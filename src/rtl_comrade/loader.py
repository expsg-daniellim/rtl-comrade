...
def load_config_file(Config, path: Path):
	...
	try:
		...
		log.fatal(
			"invalid_unicode",
			reason=e.reason,
			invalid_slice=e.object[e.start : e.end].decode(encoding=e.encoding or "utf-8", errors="replace"),
			exc_info=e,
		)
...
def load_path(path: Path) -> dict:
	...
	try:
		if os.path.isfile(path):  # Load directly if path is a file and not a dir
			...
			files = [
				PluginFileConfig(None, file, None, None)
				for file in filter(
					lambda p: os.path.isfile(p) and p.suffix == ".py",
					map(lambda p: path / p, os.listdir(path)),
				)
			]
			config = PluginConfig(files)
	except UnicodeDecodeError as e:
