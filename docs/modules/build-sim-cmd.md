...
| `run_id` | `KeyedValue[int \| None]` | run id (log/seed filename suffix) |
| `simv` | `KeyedValue[str]` | sim binary |
| `seed` | `KeyedValue[int]` | resolved seed |
...
`test` — forwarded; `command` — a `Command` writing `<test><suffix>.log`/`.err`, consumed by [run-process](run-process.md); `timeout` — `KeyedValue(key, float)`; `randseed` — a `RandSeed(key, seed, randseed_path, argv)` feeding both [write-randseed](write-randseed.md) and [link-latest](link-latest.md).
...
`sim-build`, contract `keyed_join` (`key_field: key`, `persistent_inputs: [builder_cfg, builder_mode, logs_dir]`).
