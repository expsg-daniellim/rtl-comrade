...
`test` + `run_id` + `simv` forwarded, plus `seed` — the resolved `int`. A failed replay emits nothing.

## Failure routing

In `REPLAY` mode a missing, non-integer, or unreadable `.randseed` file is caught and logged at `ERROR`.

## Graph node

`seed`, contract `keyed_join` (`key_field: key`, `persistent_inputs: [seed_mode, builder_cfg, logs_dir]`, `unwrap: true`, `ignore: [test]`). The `run_id`, `simv` and `seed` edges ride the wire as `KeyedValue`s; the contract unwraps them on the way in and keys everything the module emits back to the run on the way out, so the module never handles the key.
