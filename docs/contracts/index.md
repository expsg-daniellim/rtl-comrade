...
## Contract-layer payload types

Some contracts read or construct domain-specific values carried as payloads (distinct from `EndSentinel`, which signals stream termination):

| Type | Imported from | Used by |
|---|---|---|
| `GroupEnd` | `contracts.sentinels` | `group_until_end` — signals the end of one group while keeping the port open |
| `KeyedValue` | `contracts.sentinels` | `keyed_join` — a value tagged with a correlation key; the contract unwraps and rewraps it under `unwrap` |
...
| [keyed\_join](keyed_join.md) | Joins items from all ports by a correlation key — the `key_field` attribute or dict entry of each payload (default `"key"`). Optionally unwraps each `KeyedValue` for the module and rewraps what it emits |
