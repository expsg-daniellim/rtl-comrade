...
- `from_config` runs in five phases: build a `PreNode` per config node (module + input ports + contract recipe); wire and validate edges into `node_dsts`, per-source-node lists of id-space `Connection`s (`self_port`/`other_node`/`other_port`); static deadlock screening over `(prenodes, node_dsts)`; an inline topological pass over `(prenodes, node_dsts)` to compute each input port's control-dependence labels (each output inherits the union of its node's gating input labels plus a `(node_id, arm)` term when it belongs to a branch arm); then materialise each node's runtime dispatch map (source output port → destination `Port`s, resolved from the `Connection`s) and `Node.from_prenode` couples each `PreNode` with that map and its labels into the immutable runtime `Node`. See [branch_labels.md](branch_labels.md) and [node.md](node.md)
