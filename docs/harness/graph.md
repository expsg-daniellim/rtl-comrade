...
- contract resolution is delegated to `ContractDefinitions.from_config`, which raises rather than logs; `from_config` catches each failure type, logs it against the node index and id (`invalid_contract`, `missing_contract_function`, `missing_contract_parameter`, `invalid_contract_parameter_type`), and continues to the next node so one bad node does not mask the rest
