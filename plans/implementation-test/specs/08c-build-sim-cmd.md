...
**Depends on:** spec 03 (run-process), spec 07 (compile cycle — the `simv` edge), spec [08b](08b-resolve-seed.md) (the shared `run_suffix` helper), spec [01a](01a-builder-schema.md) (`BuildSimCmdMod` consumes `RtlBuilderConfig` methods), spec [01b](01b-suite-schema.md) (`BuildSimCmdMod` reads `test.get_timeout()`, `get_plusargs()`, `get_plusdefines()`).
...
outputs:           test     → TestConfig (self-keyed)   (forwarded to post-sim classification)
...
class BuildSimCmdMod:
    def run(self, test:TestConfig, run_id:KeyedValue[int | None], simv:KeyedValue[str], seed:KeyedValue[int], builder_cfg:RtlBuilderConfig, builder_mode:str, logs_dir:Path):
        ...
        stem = logs_dir / f"{test.get_name()}{run_suffix(run_id.value)}"   # logs_dir: resolved Path from ensure-logs-dir
        ...
        yield ("randseed", RandSeed(test.key, seed=seed.value, randseed_path=rs_path, argv=argv))   # argv carried for the hier_inst_seed check
```

## Algorithm

1. Take the compiled simv from the joined `simv` edge: `simv_path = simv.value` (born at `build-compile-cmd`, co-gated here).
2. Build plusdefines from `test.get_plusdefines()` exactly as in `build-compile-cmd`, and plusargs from `test.get_plusargs()` (spec 01b → `dict | None`): each entry `f"+{k}={v}"`, or `f"+{k}"` when `v is None`.
...
4. Resolve the timeout: `(timeout, is_custom) = test.get_timeout()` (spec 01b — `(self.timeout, True)` on a per-test override, else `(60, False)`); when `is_custom`, log `log.warning("custom_sim_timeout", …)` (rtl_buddy parity, `vlog_sim.py:233-234`); emit `timeout` as a `{key, value}` edge whose value is `float(timeout)`.
5. Compose the log/randseed paths off one stem `stem = logs_dir / f"{test.get_name()}{run_suffix(run_id.value)}"` → `log = f"{stem}.log"`, `err = f"{stem}.err"`, `randseed_path = f"{stem}.randseed"`. `logs_dir` is the resolved `Path` from `ensure-logs-dir` (join onto it — no ambient-CWD assumption). `run_suffix(run_id.value)` is the shared `sim.py` helper defined in spec [08b](08b-resolve-seed.md); it returns `""` when the `run_id` value is `None`, else `f"_{run_id:04d}"` (run-id zero-padded to four digits) — rtl_buddy `_get_log_path` (`tools/vlog_sim.py:82-86`); e.g. run-id 5 → `<logs_dir>/my_test_0005.log`. Do not `mkdir(logs_dir)` — already bootstrapped.
6. Emit four edges via the generator: `("test", test)` (forward for the post-sim classification branch); `("command", Command(test.key, argv=argv, stdout_path=log, stderr_path=err))`; `("timeout", KeyedValue(test.key, float(timeout)))`; `("randseed", RandSeed(test.key, seed=seed.value, randseed_path=randseed_path, argv=argv))`. `simv`/`run_id`/`seed` are consumed but **not** forwarded — they die here. The `argv` rides `randseed` (as well as `command`) so the downstream `keyed_join` `write-randseed` can check it for `hier_inst_seed` (spec 08d). `log`/`err` reach post-sim via `proc` (which echoes the redirect paths); `randseed_path`/`seed`/`argv` reach `write-randseed` via `randseed`.
