...
- `Graph.from_file("graphs/test.yaml")` → loads without error; every node `class_name` resolves against `modules/config.yaml` (incl. `summarise-results`, `print-summary`, `write-summary-log`) and every contract against `contracts/config.yaml` (incl. `any`) (boundary: no dangling manifest reference).
- `validation.py` on the loaded graph → reports **no** cycles and **no** overloaded inputs (single-source-per-port holds). The `results-summary` node's `contract_port_mappings` builds its 13-port input surface; all 13 terminal edges validate as destinations against that surface; the node's `table` output port validates and its two fan-out edges resolve to `print-summary.table` / `write-summary-log.table` (boundary: a `contract_port_mappings` node screened against its declared contract ports, not the `**?**` module signature).
- Running the graph → the 13 terminal `TestResult`s fan into `results-summary` via the `any` contract; its `finalise()` renders the table once at run end and emits it on `table` (no-op on an empty/list-mode run); the two sinks each fire once on that delivery. No result port logs `no_destination` (all are wired).
...
- The summary is an **in-graph module** (`summarise-results` → `SummariseResultsMod`) that emits the rendered table on `table` to two sink modules (`print-summary` → `PrintSummaryMod`, `write-summary-log` → `WriteSummaryLogMod`), all manifest entries; `test.yaml` has **no** `logging` block. The dormant `SummaryProcessor` ([10c](10c-summary-handler.md)) is not wired.
