...
| Spec | File(s) | Owns |
|---|---|---|
| [01 core](specs/01-shared-schema.md) | `root.py`, `results.py`, `seed_mode.py`, `run_depth.py`, `payloads.py`, `__init__.py` | `RootRtlField`/`PlatformConfig` + the `RootConfig` runtime holder; the single `TestResult` value object (with `@classmethod` constructors `compile_fail`/`sim_timeout`/`early_stop`/`skip`/`prep`/`parse`) + the `ResultType` enum; the `SeedMode` and `RunDepth` enums; the edge-payload dataclasses (`KeyedValue`/`Command`/`Proc`/`RandSeed`/`RandSeedDone`); the package `__init__.py` re-export surface (sole owner). |
