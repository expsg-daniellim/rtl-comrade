...
Behavioural deltas discovered or confirmed during end-to-end validation (spec 12). Each entry notes whether the delta is deliberate (per the design plan in `implementation-test/07-ambiguities-and-assumptions.md`) or an unexpected finding.
...
## Spec-local divergences (from [03]/[08e], not in [07])

These are recorded inline in their specs ("Record in `divergences.md` when implementation lands") rather than in [07]. The implementation has landed (`modules/rtl_buddy/build.py`, `sim.py`).
...
## Latent / conditional divergences (conditions not yet triggered)

Their specs gate recording on a condition that has not fired; recorded pre-emptively.
...
The validated `verif/sandbox` suite declares no hooks, and the reference `verif/template` scripts (`example_sweep.py`/`example_preproc.py`) trip neither. Specs 05f/06a gate recording on a real script tripping these; recorded pre-emptively.
