...
- `results.py` — the `ResultType` enum + the single `TestResult` value object + the preset factories. The former rtl_buddy subclass hierarchy (`TestPassResults`/`CompileFailResults`/`SimTimeoutResults`/`EarlyStopResults`/`SkipResults` over a base `TestResults`) is **collapsed** to one dataclass: those subclasses carried no behaviour — none overrode `is_pass()` — they were just named presets stamping different constant strings, so they are replaced by one `TestResult` plus factories. `ResultType` names the **originator** of a result (which stage produced it); the `result` field carries the **verdict** (`"PASS"`/`"FAIL"`/`"NA"`/`"SKIP"`). The two are orthogonal — a `PARSE` result may be `PASS`, `FAIL`, or `NA`. `TestResult` is `frozen=True` and **self-keyed** (it carries its own `key`); there is **no** separate `Result` wrapper — terminal ports emit the `TestResult` directly.
  ...
  |---|---|---|---|---|
  | `TestResult.compile_fail(key)` | `COMPILE_FAIL` | `"FAIL"` | `"Compile failed"` | interpret-compile |
  | `TestResult.sim_timeout(key)` | `SIM_TIMEOUT` | `"FAIL"` | `"Sim hit timeout"` | interpret-sim |
  | `TestResult.early_stop(key, desc)` | `EARLY_STOP` | `"NA"` | caller's | early-stop-gate |
  | `TestResult.skip(key, desc)` | `SKIP` | `"SKIP"` | caller's | filter-reglvl |
  | `TestResult.prep(key, desc)` | `PREP` | `"FAIL"` | caller's | load-model, expand-sweep, run-preproc, write-filelist, resolve-seed |
  | `TestResult.parse(key, result, desc)` | `PARSE` | caller's verdict | caller's | parse-log, parse-uvm-log |

  `TestResult.prep` replaces the old `make_fail_result` (now keyed, `type_=PREP`, verdict still `"FAIL"`). The two fixed-desc constructors (`TestResult.compile_fail`/`TestResult.sim_timeout`) own the canonical `"Compile failed"`/`"Sim hit timeout"` strings in one place.
...
- `payloads.py` — the edge-payload dataclasses [`02 — payload conventions`](../02-payload-conventions.md) catalogues as Shapes 1/2/3. The correlation field `keyed_join` matches on is named by the contract's `keyed_field` (default `"key"`, which the test graph uses throughout); `contracts/keyed_join.py::_key_of` reads it attribute-first, falling back to a dict entry. A payload carries its **own** `key` field exactly when the key is its own identity: the **multi-field messages** (cohesive, key is one of their fields), **`TestResult`** (self-keyed — its `key` is the producing test invocation's id; folded in when the old `Result` wrapper was removed), and **`TestConfig`** (its `key` *is* the scheduled-test instance — it rides the `test` edge bare, [01b](01b-suite-schema.md)). The other single-value edges ride the generic `KeyedValue[T]` envelope because the key is *not* the value's own identity — a primitive (`int`/`Path`/`str`) has no identity, and `model` is keyed by the **test's** identity (so `write-filelist` can join it back), foreign to the `ModelConfig`. The set:
  ...
  | `TestResult` | `key: str`, `type_: ResultType`, `result: str`, `desc: str` | yes | Shape 3 (terminal, self-keyed) | every terminal port |
...
- `TestResult` is one `frozen=True` self-keyed dataclass (`key`, `type_: ResultType`, `result`, `desc`) — no subclass hierarchy, no `Result` wrapper. Each `@classmethod` constructor (`TestResult.compile_fail`/`TestResult.sim_timeout`/`TestResult.early_stop`/`TestResult.skip`/`TestResult.prep`/`TestResult.parse`) produces the `type_`/`result`/`desc` combination in its row of the table above. `ResultType` is a closed enum with exactly `PARSE`/`COMPILE_FAIL`/`SIM_TIMEOUT`/`PREP`/`EARLY_STOP`/`SKIP`.
