...
- `results.py` — the `ResultType` enum + the single `TestResult` value object + the preset factories. The former rtl_buddy subclass hierarchy (`TestPassResults`/`CompileFailResults`/`SimTimeoutResults`/`EarlyStopResults`/`SkipResults` over a base `TestResults`) is **collapsed** to one dataclass: those subclasses carried no behaviour — none overrode `is_pass()` — they were just named presets stamping different constant strings, so they are replaced by one `TestResult` plus factories. `ResultType` names the **originator** of a result (which stage produced it); the `result` field carries the **verdict** (`"PASS"`/`"FAIL"`/`"NA"`/`"SKIP"`). The two are orthogonal — a `PARSE` result may be `PASS`, `FAIL`, or `NA`. `TestResult` is `frozen=True` and **self-keyed** (it carries its own `key`); there is **no** separate `Result` wrapper — result ports emit the `TestResult` directly.

  ```python
  class ResultType(Enum):
      PARSE = "parse"               # parse-log / parse-uvm-log verdict (result ∈ PASS/FAIL/NA)
      COMPILE_FAIL = "compile_fail" # interpret-compile
      SIM_TIMEOUT = "sim_timeout"   # interpret-sim (timeout)
      PREP = "prep"                 # generic per-test pipeline failure (load-model/…/resolve-seed)
      EARLY_STOP = "early_stop"     # early-stop-gate
      SKIP = "skip"                 # filter-reglvl

  @dataclass(frozen=True)
  class TestResult:
      key: str
      test_name: str   # the test's get_name() — the summary table's first column (rtl_buddy parity)
      type_: ResultType
      result: str   # verdict: "PASS" | "FAIL" | "NA" | "SKIP"
      desc: str
      def is_pass(self) -> bool:
          return self.result in ("PASS", "SKIP")
  ```

  `is_pass()` returns `True` for `PASS`/`SKIP` only. `ResultType` is a closed enum (one member per result originator), mirroring the `SeedMode`/`RunDepth` pattern; `TestPassResults`'s old `"Generic test pass"` preset is dropped — nothing constructed it (parse-log/parse-uvm-log build the verdict inline as a `PARSE` result). `@classmethod` constructors on `TestResult` build the canonical originator/verdict/desc combinations so call sites don't repeat the constant strings:

  | constructor | `type_` | `result` | `desc` | used by |
  |---|---|---|---|---|
  | `TestResult.compile_fail(key, test_name)` | `COMPILE_FAIL` | `"FAIL"` | `"Compile failed"` | interpret-compile |
  | `TestResult.sim_timeout(key, test_name)` | `SIM_TIMEOUT` | `"FAIL"` | `"Sim hit timeout"` | interpret-sim |
  | `TestResult.early_stop(key, test_name, desc)` | `EARLY_STOP` | `"NA"` | caller's | early-stop-gate |
  | `TestResult.skip(key, test_name, desc)` | `SKIP` | `"SKIP"` | caller's | filter-reglvl |
  | `TestResult.prep(key, test_name, desc)` | `PREP` | `"FAIL"` | caller's | load-model, expand-sweep, run-preproc, write-filelist, resolve-seed |
  | `TestResult.parse(key, test_name, result, desc)` | `PARSE` | caller's verdict | caller's | parse-log, parse-uvm-log |

  Every constructor takes `test_name` (the test's `get_name()`) as its second positional after `key` and stores it on the frozen `TestResult` — the in-graph summary node ([10d](10d-summarise-results.md)) reads it directly as the table's first column. The terminal sites already compute `test.get_name()` for their `log.*("…", test_name=…)` calls ([05 — Log idioms](../05-branching-and-results.md#log-idioms-per-failure-site)), so they pass the same value to the constructor. `TestResult.prep` replaces the old `make_fail_result` (now keyed, `type_=PREP`, verdict still `"FAIL"`). The two fixed-desc constructors (`TestResult.compile_fail`/`TestResult.sim_timeout`) own the canonical `"Compile failed"`/`"Sim hit timeout"` strings in one place.
...
- `payloads.py` — the edge-payload dataclasses [`02 — payload conventions`](../02-payload-conventions.md) catalogues as Shapes 1/2/3. The correlation field `keyed_join` matches on is named by the contract's `keyed_field` (default `"key"`, which the test graph uses throughout); `contracts/keyed_join.py::_key_of` reads it attribute-first, falling back to a dict entry. A payload carries its **own** `key` field exactly when the key is its own identity: the **multi-field messages** (cohesive, key is one of their fields), **`TestResult`** (self-keyed — its `key` is the producing test invocation's id; folded in when the old `Result` wrapper was removed), and **`TestConfig`** (its `key` *is* the scheduled-test instance — it rides the `test` edge bare, [01b](01b-suite-schema.md)). The other single-value edges ride the generic `KeyedValue[T]` envelope because the key is *not* the value's own identity — a primitive (`int`/`Path`/`str`) has no identity, and `model` is keyed by the **test's** identity (so `write-filelist` can join it back), foreign to the `ModelConfig`. The set:
...
- `TestResult.is_pass()` matches rtl_buddy semantics exactly (`True` for `PASS`/`SKIP` only; table in [02](../02-payload-conventions.md)), reading the flat `result` field.
- `TestResult` is one `frozen=True` self-keyed dataclass (`key`, `test_name`, `type_: ResultType`, `result`, `desc`) — no subclass hierarchy, no `Result` wrapper. Each `@classmethod` constructor (`TestResult.compile_fail`/`TestResult.sim_timeout`/`TestResult.early_stop`/`TestResult.skip`/`TestResult.prep`/`TestResult.parse`) takes `test_name` as its second positional and produces the `type_`/`result`/`desc` combination in its row of the table above. `ResultType` is a closed enum with exactly `PARSE`/`COMPILE_FAIL`/`SIM_TIMEOUT`/`PREP`/`EARLY_STOP`/`SKIP`.
- Every payload dataclass here is `frozen=True` (`KeyedValue` included — frozen blocks rebinding `.value`, not mutating the `TestConfig` it points at) and exposes the correlation field `keyed_join` reads via `_key_of` (default `keyed_field="key"`, attribute-first) — `TestResult` self-keys on its own `key`. `KeyedValue` is `Generic[T]`. A hand-constructed instance of each round-trips its fields, and `run-preproc`-style in-place mutation of a wrapped `TestConfig` succeeds through the frozen envelope.
...
- `TestResult.is_pass()` must return `True` for `PASS`/`SKIP` only — never for FAIL / NA / timeout / compile-fail / early-stop.
