...
`(early_stop:str="post", **edges)` with module `Config` containing `phase:str` (one of `pre`/`comp`/`sim`). One module serves all three node instances, which differ only by `config.phase` and the edges wired to them: `gate-pre` `{test}` (contract `default`); `gate-comp` `{test, simv}` and `gate-sim` `{test, proc}` (both `keyed_join`). Identity comes from `edges["test"]` (always present). Behaviour — the `early_stop` guard, the `pre < comp < sim < post` stop/forward branch, and the co-gating — is specified under [Algorithm](#algorithm) and [Constraints](#constraints). The `stop` port is wired to `results-summary` (spec [10d](10d-summarise-results.md)).
...
- Tests pass.
- Co-gating exercised across all four `early_stop` values for each `phase`: on "go" the gate forwards **every** input edge on its same-named port (`{test}` at gate-pre, `{test, simv}` at gate-comp, `{test, proc}` at gate-sim); a `stop` drops them all and emits the `stop` `TestResult`.
