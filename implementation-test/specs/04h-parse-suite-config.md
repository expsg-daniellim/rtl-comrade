...
The skeleton folds steps 1–4 into the `SuiteConfig(test_config_path)` constructor; the constructor
flow is enumerated in spec [01b — `SuiteConfig`](01b-suite-schema.md) and reproduced here.

1. Open and deserialise `test_config_path` into the raw `SuiteConfigFile` (spec 01b):
   `from_yaml(SuiteConfigFile, test_config_path.read_text())`.
...
3. Stamp `suite_dir = test_config_path.parent` onto each test so `load-model` (spec 05e) can later
...
- `ParseSuiteConfigMod` — reads `test_config_path:Path` (the resolved path from
