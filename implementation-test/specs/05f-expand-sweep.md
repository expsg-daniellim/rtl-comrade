...
- **Script-compatibility risk (KIV).** The drop-in promise covers config *files*; sweep scripts are *code*. The `exec` namespace injects **this plan's reimplemented** `TestConfig`/`root_cfg` (`test_cfg`, `TestConfig`, `root_cfg`), not rtl_buddy's. A sweep script written against rtl_buddy's `TestConfig` surface can break here — most notably the `model` → `model_name` rename (spec [01b — Notable divergences](01b-suite-schema.md), divergence 2): a script reading the pre-`initialise` `test_cfg.model` string finds `model_name` instead, and `test_cfg.model` is now `None` until `load-model`. rtl_buddy's `AGENTS.md` flags hook scripts (`sweep`/`preproc`/`postproc`) as **compatibility-sensitive APIs**, so this is a genuine behavioural delta, not a config-schema one. Record it in [07 — Notable divergences](../07-ambiguities-and-assumptions.md) and validate real sweep scripts against the reimplemented `TestConfig` before claiming parity. `run-preproc` (spec [06a](06a-run-preproc.md)) carries the same risk.
