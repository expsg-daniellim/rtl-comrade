...
- `test` with a sweep script that raises (e.g. `raise RuntimeError("boom")`) → yields `("fail", Result(key, <FAIL with str(e)>))`, neither `test` nor `model`, `logging_handler.failure is True`, no `typer.Exit`; `test.model` is restored (the `finally`).
...
- `test` with a sweep script that sets `out_test_cfgs` to a non-iterable (e.g. an `int`), or appends an object that rejects `.key` assignment → the fan-out raises `TypeError`/`AttributeError`, caught by the same guard → yields `("fail", Result(key, <FAIL>))`, `log.error`, no abort (boundary: malformed script output routed like a script error, not bubbled to the harness).
