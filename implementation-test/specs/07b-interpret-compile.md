...
Route the compile result on the subprocess rc — forward `test`+`simv` on success, emit `CompileFailResults` on failure.
...
- `proc.rc == 2` with a stderr file → emits `("fail", Result(key, CompileFailResults))` (no `test`/`simv`), `logging_handler.failure is True`, and the ERROR `compile_failed` log carries `rc`/`stderr_path`/the stderr tail **and `result="FAIL"`/`desc`** (so `SummaryProcessor` collects it).
