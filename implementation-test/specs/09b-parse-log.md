...
4. **Log the verdict directly, then emit.** One `test_result` event per verdict: `log.error("test_result", key=, result=, desc=)` when `not result.is_pass()` (FAIL **and NA** — the ERROR is the deferred-exit driver), else `log.info("test_result", ...)` (PASS) — the same pattern `early-stop-gate` (`10a`) uses. Then return `("default", {"key": test["key"], "result": result})`. `SummaryProcessor` watches `test_result` and renders the row; the `default` port stays unwired.
