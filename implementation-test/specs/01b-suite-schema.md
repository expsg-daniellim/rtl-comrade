...
2. **Field rename `model` → `model_name`** on runtime `TestConfig` (raw stays `model` for YAML compat). Avoids the rtl_buddy name collision where `TestConfigFile.model: str` becomes `TestConfig.model: ModelConfig` after `initialise`.
...
- **`get_reglvl` resolution order.** Four cases: int `_reglvl` → that int regardless of builder; dict with builder key → that entry; dict with `default` only → default; `None` → `0`; dict missing both builder and `default` → `log.fatal` (caplog + bubbling-`SystemExit`).
