...
**Depends on:** spec 06 (write-filelist), spec [01a](01a-builder-schema.md) (`BuildCompileCmdMod` consumes `RtlBuilderConfig` methods), spec [01b](01b-suite-schema.md) (`BuildCompileCmdMod` reads `test["value"].get_name()`/`get_plusdefines()`).
...
outputs:           test    → {key, value}   (forwarded)
...
class BuildCompileCmdMod:
    def run(self, test, filelist, builder_cfg, logs_dir, work_dir, builder_mode:str = "debug"):
        ...
        yield ("simv", { "key": test["key"], "value": simv })      # simv born as its own edge
...
1. Derive tags and the verilator switch: `test_tag = re.sub(r"[^A-Za-z0-9_.-]", "_", test["value"].get_name())`, `exe = builder_cfg.get_exe()`, `is_verilator = os.path.basename(exe).startswith("verilator")`, `build_dir = str(Path(work_dir) / f"obj_dir_{test_tag}")` (rooted on the validated base dir from `check-suite-cwd`, not the ambient CWD), and `simv = f"{build_dir}/simv" if is_verilator else builder_cfg.get_simv()` (the caller-side verilator quirk, spec 01a; the verilator `simv` inherits the `work_dir` rooting via `build_dir`).
2. Build the plusdefines list from `test["value"].get_plusdefines()` (spec 01b → `dict | None`): when not `None`, format each entry `f"+define+{k}={v}"`, or `f"+define+{k}"` when `v is None`.
3. Assemble the argv: `[exe, *builder_cfg.get_compile_time_opts(builder_mode)]`, then append `["--Mdir", build_dir]` when `is_verilator`, then `[*plusdefines, "-f", str(filelist["value"])]`. Do not `mkdir(logs_dir)` — `ensure-logs-dir` already created it.
4. Emit three edges via the generator: `("test", test)` (forward unchanged), `("simv", {"key": test["key"], "value": simv})` (the compiled simv path — born here as its own edge, **not** folded into the bag; `build_dir` is not emitted, unused downstream), and `("command", {"key": test["key"], "argv": argv, "stdout_path": str(logs_dir / f"{test_tag}.compile.log"), "stderr_path": str(logs_dir / f"{test_tag}.compile.err")})`. `logs_dir` is the resolved `Path` from `ensure-logs-dir`; join filenames onto it (`logs_dir / name`) — do not assume a CWD-relative `"logs"`.
...
- `BuildCompileCmdMod` — `(test, filelist, builder_cfg, logs_dir:Path, work_dir:Path, builder_mode:str="debug")`, `keyed_join` over `test` + `filelist` (joined by key) with the config singletons as `persistent_inputs` → assembles the argv as `[builder_cfg.get_exe()] + builder_cfg.get_compile_time_opts(builder_mode) + (["--Mdir", build_dir] if is_verilator else []) + plusdefines + ["-f", filelist["value"]]`, where `is_verilator = os.path.basename(builder_cfg.get_exe()).startswith("verilator")` (the caller-side verilator switch documented in spec [01a — Verilator quirk](01a-builder-schema.md)). Computes `test_tag = re.sub(r"[^A-Za-z0-9_.-]", "_", test["value"].get_name())`, `build_dir = str(Path(work_dir) / f"obj_dir_{test_tag}")` (rooted on the validated base directory `work_dir` from `check-suite-cwd` — the same artefact-location provider `ensure-logs-dir` consumes; **load-bearing** persistent input), and `simv = f"{build_dir}/simv" if is_verilator else builder_cfg.get_simv()` (mirrors `rtl_buddy/src/rtl_buddy/tools/vlog_sim.py:61-80`; the verilator `simv` inherits the `work_dir` rooting via `build_dir`). Emits `simv` as **its own `{key, value}` edge** (no longer folded into a bag); does not emit `build_dir` (not needed downstream). Does not `mkdir(logs_dir)` — `ensure-logs-dir` has already bootstrapped the directory. `plusdefines` is built from `test["value"].get_plusdefines()` (spec [01b](01b-suite-schema.md) — returns `dict | None`; when not `None`, format each entry as `f"+define+{k}={v}"` or `f"+define+{k}"` for `v is None`, mirroring `vlog_sim.py:107-117`). Compile log paths are composed by joining onto the resolved `logs_dir` `Path` supplied by `ensure-logs-dir`: `str(logs_dir / f"{test_tag}.compile.log")` and `.compile.err`. This module never references the ambient CWD — `logs_dir` already encodes the artefact location decided once by `check-suite-cwd`/`ensure-logs-dir`. Emits:
  - `("test", test)` — forwards the test edge unchanged
  - `("simv", {"key", "value": simv})` — the compiled simv path, born here
...
- Non-verilator builder, `get_plusdefines()` is `None` → yields `("test", test)`, `("simv", {"key", "value": builder_cfg.get_simv()})`, then `("command", {argv, stdout_path, stderr_path})` where `argv == [exe, *compile_opts, "-f", str(filelist["value"])]` (matches `VlogSim.compile`).
...
- `test["value"].get_name()` has shell-unsafe chars → `test_tag` is sanitised in `build_dir`, verilator `simv`, and both log paths (boundary: `test_tag` regex).
...
- Emit `simv` as its own `{key, value}` edge (`("simv", {"key": test["key"], "value": simv})`); do **not** emit `build_dir` (unused downstream).
...
**Input pairing (resolved by the split).** This node now `keyed_join`s `test` + `filelist` **by key**, so the positional/lockstep-pairing fragility the bag design carried here is gone — there is no reliance on arrival order, and inserting a node between `write-filelist` and this one can no longer mismatch the streams. The same resolution applies to `build-sim-cmd` (spec [08c](08c-build-sim-cmd.md)), which `keyed_join`s `test`+`run_id`+`simv`+`seed`. (The persistent config singletons ride the `keyed_join`'s `persistent_inputs`, the capability commit `3068cda` added.)
